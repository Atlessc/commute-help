...
- [ ] Produce the first full historical MOSS station-prediction matrix on calibration dates. The ODOT-backed V1 calibration-only temporal selector failed because no station had all 1,920 healthy date/bucket rows. Forensic lineage closes exactly: 707,052 pre-health station-buckets minus 17,436 health-v4 UNKNOWN omissions equals the 689,616-row calibration station parquet; an additional 82,068 Cartesian identities were already absent upstream. The health-v4 calibration station surface remains the appropriate station-level source, but the zero-missing complete-case assumption was unrealistic. V2/panel selection and coverage thresholds are paused and not authored. Regional clock, final pre-roll, historically eligible replay input, residuals, and demand remain unselected/not started.
...
- [ ] **Build the first historical MOSS prediction matrix.** Preserve the V1 selector failure and the static healthy-frame forensic audit. The audited health-v4 station parquet is calibration-only, full-day collectively, and not narrowed by operator eligibility, stable-core, route-observability, or MOSS-supported-station intersections. Masked station-buckets are physically omitted as UNKNOWN; operator-ineligible rows remain present. No downstream row-loss bug is visible. V2 is deliberately paused until a missingness-aware fixed-composition station-panel decision protocol is defined without importing diagnostic coverage thresholds as methodology.
